...
% \title[Байесовское выравнивание нейросетевых моделей]{Байесовское выравнивание нейросетевых моделей}
\author{А.\,В.\,Грабовой}
...
\date[2022]{\small 23\;марта\;2022\,г.}
...
%\\%\item  
%2)~метод выравнивания структур параметрических моделей,% на основе байесовского вывода,
%\\%\item  
...
%\justifying
%связного байесовского вывода. 
%Для назначения априорного распределения параметров при выборе моделей глубокого обучения используются %ется априорная экспертная информация и
%апостериорные распределения параметров ранее полученных моделей.
%\end{block}
%}
%\end{frame}
%\end{document}
...
\item[] Выборка
    ...
    \item[] Оптимизационная задача
        ...
        \only<2>{
            \[
            \hat{\mathbf{w}}, \hat{\mathbf{A}}_{w} = \mathop{\arg\min}_{\mathbf{w} \in \mathbb{R}^{n_{w}}, \mathbf{A} \in \mathbb{R}^{n_{w}\times n_{w}}} \mathcal{L}\bigr(\mathbf{w},\mathbf{A}_{\text{w}}|\mathfrak{D}, {\color{blue}\mathbf{w}_0, \mathbf{A}_0}, \mathbf{g}\bigr)
            \]}
        \only<3>{
            \[
            \hat{\mathbf{w}}, \hat{\mathbf{A}}_{w} = \mathop{\arg\min}_{\mathbf{w} \in \mathbb{R}^{n_{w}}, \mathbf{A} \in \mathbb{R}^{n_{w}\times n_{w}}} \mathcal{L}\bigr(\mathbf{w},\mathbf{A}_{\text{w}}|{\color{blue} \mathbf{s}}, \mathfrak{D},\mathbf{w}_0, \mathbf{A}_0, \mathbf{g},{\color{blue}\mathbf{f}}\bigr)
            \]}
\end{itemize}
\end{frame}
...
~\\[-8mm]
\begin{figure}[h!]
\subfloat[Истинное распределение]{\includegraphics[width=0.3\textwidth]{figures/distr_real.png}}
\subfloat[Модель без учителя]{\includegraphics[width=0.3\textwidth]{figures/distr_without_teacher.png}}
\subfloat[Модель с учителем]{\includegraphics[width=0.3\textwidth]{figures/distr_with_teacher.png}}
\end{figure}
...
\begin{comment}
%---------------------------------------------------------------------------------
\begin{frame}{Анализ решения задачи выравнивания структур моделей с разным размером скрытых слоев}
...
\begin{tabular}{|l|c|c|c|c|}\cline{1-5}
                  & Учитель           & Ученик        & Дистил.-ученик & distil-student-all \\ \cline{1-5}
...
%\frametitle<3>{Байесовская дистилляция модели}
\frametitle<1>{Задача оптимизации параметров модели}
\frametitle<2>{Бейесовский выбор модели}
\frametitle<3>{Байесовская дистилляция модели}
